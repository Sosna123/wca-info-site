...
function formatNewMbf(time: string): string {
    ...
    timeToSolve =
        timeArr.length === 10
            ? Number(`${timeArr[3]}${timeArr[4]}${timeArr[5]}${timeArr[6]}${timeArr[7]}`)
            : Number(`${timeArr[3]}${timeArr[4]}${timeArr[5]}${timeArr[6]}`);
    ...
    seconds =
        timeToSolve >= 3600 ? Math.trunc(timeToSolve - (hours * 3600 + minutes * 60)) : timeToSolve - minutes * 60;
    secondsStr = seconds.toString().length === 1 ? `0` + seconds.toString() : seconds.toString();

    result =
        timeToSolve >= 3600
            ? `${solved}/${attempted} ${hours}:${minutesStr}:${secondsStr}`
            : `${solved}/${attempted} ${minutesStr}:${secondsStr}`;
...
function formatOldMbf(time: string): string {
    ...
    timeToSolve =
        timeArr.length === 10
            ? Number(`${timeArr[5]}${timeArr[6]}${timeArr[7]}${timeArr[8]}${timeArr[9]}`)
            : Number(`${timeArr[2]}${timeArr[3]}${timeArr[4]}${timeArr[5]}${timeArr[6]}`);

    solved =
        timeArr.length === 10 ? 99 - Number(`${timeArr[1]}${timeArr[2]}`) : 99 - Number(`${timeArr[0]}${timeArr[1]}`);
